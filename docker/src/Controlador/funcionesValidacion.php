...
function validarCodigoEspectaculo($codigoEspectaculo)
{
...
{
    global $patronFecha;
    $esValido = preg_match($patronFecha, $fecha);
    return $esValido ? $fecha : false;
}


function validarCodigo($codigo, $conexionBD)
{
    $esValido = false;
...
function validarUsuario($usuario, $conexionBD)
{
    $esValido = false;
    $usuario = validarCadena($usuario);
    if (noExisteUsuario($usuario, $conexionBD)) {
        $esValido = true;
    }
    return $esValido ? $usuario : false;
}

function validarContraseña($contraseña, $conexionBD)
{
    $esValido = false;
    $contraseña = validarCadena($contraseña);
    if (noExisteContraseña($contraseña, $conexionBD)) {
        $esValido = true;
    }
    return $esValido ? $contraseña : false;
}

function validarCorreo($correo, $conexionBD)
{
    $esValido = false;
    $correo = validarEmail($correo);
    if (noExisteCorreo($correo, $conexionBD)) {
        $esValido = true;
    }
    return $esValido ? $correo : false;
}

function validarDireccion($direccion, $conexionBD)
{
    $esValido = false;
    $direccion = validarCadena($direccion);
    if (noExisteDireccion($direccion, $conexionBD)) {
        $esValido = true;
    }
    return $esValido ? $direccion : false;
}

function validarFechaNacimiento($fecha, $conexionBD)
{
    $esValido = false;
    $fecha = validarFecha($fecha);
    if (noExisteFechaNacimiento($fecha, $conexionBD)) {
        $esValido = true;
    }
    return $esValido ? $fecha : false;
}
...
function validarContraseñaExistente($contraseña, $conexionBD)
{
    $esValido = false;
    $contraseña = validarCadena($contraseña);
    if (existeContraseña($contraseña, $conexionBD)) {
        $esValido = true;
    }
    return $esValido ? $contraseña : false;
}
